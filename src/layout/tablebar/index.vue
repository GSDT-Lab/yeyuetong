  ...
  <div class="table" :class="{value: LayOutSettingStore.fold ? true : false}">
    <div class="left1">
      ...
      <el-breadcrumb separator-icon="ArrowRight">
        <el-breadcrumb-item v-for="(item,index) in $route.matched" :key="index" v-show="item.meta.index!=0" :to="item.path">
    ...
    <div class="right123">
      ...
      <el-popover placement="bottom" title="消息提醒" width="300px" trigger="click">
        <el-form>
          <el-form-item label="新增规划审批数量:">
            <span>{{ a }}</span>
            <el-button @mousedown="e => e.preventDefault()" type="primary" size="small" style="margin-left: 40px;" @click="goone()">前往</el-button>
          </el-form-item>
          <el-form-item label="新增活动审批数量:">
            <span>{{ b}}</span>
            <el-button @mousedown="e => e.preventDefault()" type="primary" size="small" style="margin-left: 40px;"  @click="gotwo()">前往</el-button>
          </el-form-item>
        </el-form>
        <template #reference>
      ...
      <img src='/public/img/head1.jpg' style="width: 32px; height: 32px; margin-left: 10px; margin-right: 10px; border-radius: 50%;">
      <el-dropdown>
...
console.log(result)
const jk = await result.data[0] + result.data[3]
a.value = result.data[0]
b.value = result.data[3]
...
.user-name, .username {
  color: #222 !important;
  font-weight: 700;
  text-shadow: 0 1px 4px rgba(255,255,255,0.2), 0 1px 8px rgba(0,0,0,0.08);
  font-size: 16px;
  letter-spacing: 1px;
}
</style>
