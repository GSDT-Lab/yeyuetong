...
import {Planning_sendcomplete, Planning_sendyour_complete} from '@/api/planning/index'
...
for (let i = 0; i < result.data.length; i++) {
  ...
  text.on('mouseover', function (e) {
        if (tooltip.value) {
          tooltip.value.innerHTML = `网格编码:${item.encode}<br>网格评分:${item.pinfeng}<br>网格灯光指数:${item.dengguang}<br>网格人流量:${item.renliu}<br>网格内摄像头数量:${item.shexiangtou}<br>网格噪声分贝:${item.zaosheng}`; // Set text to tooltip
...
const save=()=>{
  ...
  Planning_sendyour_complete(userParams)
