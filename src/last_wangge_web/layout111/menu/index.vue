<template>
  <template v-for="(item, index) in menuList" :key="item.path" >
    <template v-if="!item.children">
      <el-menu-item @click="goRoute" v-if="item.meta.index == 2" :index="item.path" collapse>
        <el-icon>
    ...
    <template v-if="item.children && item.children.length == 1&&item.meta.index == 2">
    ...
    <!--  子路由大于1个 -->
    <el-sub-menu :index="item.path" v-if="item.children && item.children.length>1&&item.meta.index == 2"  collapse>
      <template #title>
        <el-icon>
          <component :is="item.meta.icon"></component>
        </el-icon>
